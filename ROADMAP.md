...
- **[EM ABERTO] `transactions.queue` acumula sem consumidor no modo `queue`**: as duas filas de pedido (`transactions.queue` e `transactions.queue.decision.requests`) estão ligadas à mesma routing key `transaction.created`, mas só uma tem consumidor por vez — a outra depende do `TransactionWorker`, que não sobe no modo `queue`. Medido com `rabbitmqctl list_queues`: 100 mensagens paradas em `transactions.queue` com 0 consumers enquanto `decision.requests` drenava normalmente.
  - **O que prejudica**: memória do broker, não integridade — nenhuma decisão é perdida, o `decision.requests` recebe cópia de tudo. Em execução longa num só modo, porém, a fila órfã cresce indefinidamente.
  - **Por que ficou assim**: manter as duas ligadas torna a troca de transporte reversível — ao voltar para `http`, o worker encontra o que chegou no intervalo em vez de um buraco. É uma escolha de conveniência de PoC, não um descuido.
  - **Correção**: fazer o `bindQueue` condicional ao transporte ativo (`unbindQueue` do caminho inativo no bootstrap), ou dar TTL à fila órfã via `x-message-ttl`. A primeira é mais correta; a segunda é mais barata.
